# Apply Business Metrics SQL Migration
...
### 1. **Business Metrics Cards** (Top of Analytics Page)
   - **Avg Order Value**: Average total_price across all reservations
   - **Conversion Rate**: % of reservations that get picked up
   - **Revenue per Pickup**: Average revenue per completed pickup
   - **Active Now**: Count of currently active reservations

### 2. **Revenue Trends Chart** (Line Chart)
   - Daily revenue for last 30 days (green line)
   - Daily pickup count (blue line, right axis)
   - Shows correlation between pickups and revenue

### 3. **Reservation Funnel Chart** (Bar Chart)
   - Counts by status: ACTIVE, PICKED_UP, CANCELLED
   - Visualizes conversion flow
   - Color-coded: Blue (Active), Green (Picked Up), Red (Cancelled)

### 4. **User Growth Chart** (Existing, moved down)
   - New users per day
...
The analytics page now provides comprehensive business intelligence:
- **Revenue insights**: Daily trends, average values
- **Conversion metrics**: How many reservations convert to pickups
- **Operational data**: Active reservations needing attention
- **Funnel visualization**: Where users drop off in the reservation process
