-- ============================================
-- VERIFY AND RE-APPLY FIX
-- ============================================
-- Purpose: Check if function was updated and re-apply if needed
-- ============================================

-- Step 1: Check current function definition
SELECT 
  'CURRENT FUNCTION:' as status,
  prosrc
FROM pg_proc 
WHERE proname = 'create_reservation_atomic';

-- Step 2: Check if penalty check includes 'suspension' filter
SELECT 
  'HAS SUSPENSION FILTER:' as status,
  CASE 
    WHEN prosrc LIKE '%penalty_type = ''suspension''%' THEN 'YES - Function is updated ✅'
    ELSE 'NO - Function still has old logic ❌'
  END as result
FROM pg_proc 
WHERE proname = 'create_reservation_atomic';

-- Step 3: Re-apply the fix (run this if result above is NO)
...
BEGIN
  ...
  -- ✅ CRITICAL FIX: Only block reservations for SUSPENSIONS, not WARNINGS
  SELECT EXISTS(
...
SELECT 
  'VERIFICATION AFTER UPDATE:' as status,
  CASE 
    WHEN prosrc LIKE '%penalty_type = ''suspension''%' THEN '✅ FIXED - Warnings now allowed!'
    ELSE '❌ FAILED - Please contact support'
  END as result
FROM pg_proc 
WHERE proname = 'create_reservation_atomic';
