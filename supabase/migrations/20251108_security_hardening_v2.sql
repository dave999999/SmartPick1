...
-- FIXES: Policy conflicts and achievement claiming
-- ============================================================================
...
-- Keep public read access (everyone can view images)
...
-- NOTE: claim_achievement MUST remain accessible to authenticated users
-- so they can claim their achievements from the UI
-- The function has built-in validation to prevent abuse
...
-- ============================================================================
-- 5. SECURE CLAIM_ACHIEVEMENT FUNCTION (Users can claim their own)
-- ============================================================================

-- Update claim_achievement to ensure users can only claim their own achievements
...
BEGIN
  ...
  -- Find the user_achievement record
  SELECT * INTO v_unlock
  FROM user_achievements
  WHERE id = p_achievement_id AND user_id = v_user_id;

  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Achievement not found or not unlocked for this user');
  END IF;

  -- SECURITY: Verify the achievement belongs to the calling user
  IF v_unlock.user_id != v_user_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'Cannot claim another user''s achievement');
  END IF;

  -- Get the achievement definition for reward points
  SELECT * INTO v_def
  ...
  v_already_awarded := COALESCE(v_unlock.reward_claimed, false);
  ...
  SET reward_claimed = true, reward_claimed_at = now(), points_awarded = v_def.reward_points
  WHERE id = p_achievement_id AND user_id = v_user_id;

  -- Award the points using add_user_points (with service_role context)
  ...
  RETURN jsonb_build_object(
    ...
    'balance', COALESCE(v_tx->>'balance','')
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Ensure authenticated users can claim achievements
...
-- ✅ Points functions (add/deduct) revoked from authenticated users
-- ✅ Points functions now validate caller role internally
-- ✅ Achievement claiming KEPT for authenticated users (they can claim their own)
-- ✅ claim_achievement validates user can only claim their own achievements
--
-- Security improvements:
-- - Prevents storage quota exhaustion
-- - Prevents unauthorized image uploads
-- - Prevents direct points manipulation
-- - Users can still claim achievements from UI (secure)
-- - Enforces backend-only points add/deduct operations
-- ============================================================================
