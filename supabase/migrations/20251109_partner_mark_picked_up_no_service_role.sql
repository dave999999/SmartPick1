-- Simpler approach: Just update reservation status directly without triggering point modifications
-- The reservation status change doesn't actually modify points - points are only modified when user confirms pickup
...
BEGIN
  ...
  -- We're only changing status, not modifying any points
  UPDATE reservations
  SET 
    status = 'PICKED_UP',
    picked_up_at = NOW()
...
'Allows partner to mark their reservation as picked up. Uses SECURITY DEFINER to bypass RLS. Only updates status, does not modify points.';
