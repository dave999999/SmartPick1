-- Fix race conditions in create_reservation_atomic
-- Add penalty, ban, and active reservation count checks inside the atomic transaction
-- This prevents users from bypassing restrictions via concurrent requests
...
BEGIN
  ...
  -- ✅ RACE CONDITION FIX 1: Check user ban status within transaction
  ...
  -- ✅ RACE CONDITION FIX 2: Check penalty status within transaction
  -- Lock penalty records to prevent concurrent penalty application
  SELECT EXISTS(
  ...
  -- ✅ RACE CONDITION FIX 3: Check active reservation count within transaction
  -- Lock user's active reservations to prevent concurrent reservation creation
  SELECT COUNT(*) INTO v_active_count
  FROM reservations
  WHERE customer_id = v_customer_id 
  AND status = 'ACTIVE';
  -- Note: FOR UPDATE OF requires table alias, so we lock via the user row lock above

  IF v_active_count >= 1 THEN -- MAX_ACTIVE_RESERVATIONS = 1
  ...
  -- ✅ RACE CONDITION FIX 4: Validate quantity against user's max limit
  IF v_max_reservation_quantity IS NULL THEN
    v_max_reservation_quantity := 3; -- Default MAX_RESERVATION_QUANTITY
  END IF;
  ...
  -- Deduct points from user (with FOR UPDATE lock to prevent concurrent point changes)
...
COMMENT ON FUNCTION public.create_reservation_atomic(UUID, INTEGER, TEXT, NUMERIC, TIMESTAMPTZ) IS
  'Atomically creates a reservation with full race condition protection. Checks penalties, ban status, active reservation count, deducts points, and decrements offer quantity all within a single transaction with proper row locks.';
