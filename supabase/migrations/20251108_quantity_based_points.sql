-- Update points cost to be 5 points per unit (quantity-based)
-- Previously was fixed 15 points, now scales: 1 unit = 5 pts, 2 units = 10 pts, 3 units = 15 pts
...
DECLARE
  ...
  v_result JSON;
  v_customer_id UUID;
  v_points_cost INT; -- Now calculated based on quantity
...
COMMENT ON FUNCTION public.create_reservation_atomic(UUID, INTEGER, TEXT, NUMERIC, TIMESTAMPTZ) IS
  'Atomically creates a reservation, decrements offer quantity, and deducts points from user (5 points per unit). Refunds points if reservation fails.';
