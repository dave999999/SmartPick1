...
export default function ReservationModal({
  ...
  const [showSlotUnlockModal, setShowSlotUnlockModal] = useState(false);
  const [insufficientPoints, setInsufficientPoints] = useState(false);
  const [userMaxSlots, setUserMaxSlots] = useState(3); // Default 3, loaded from DB
  ...
  const loadUserMaxSlots = async () => {
    if (!user) return;
    try {
      const maxSlots = await getUserMaxSlots(user.id);
      setUserMaxSlots(maxSlots);
      logger.log('User max slots loaded:', maxSlots);
    } catch (error) {
      logger.error('Error loading user max slots:', error);
      setUserMaxSlots(3); // Fallback to default
    }
  };

  const handleSlotUnlockSuccess = () => {
    loadUserMaxSlots(); // Reload max slots
    loadPointsBalance(); // Reload balance
    toast.success('Slot unlocked! Your max reservation quantity increased.');
  };
  ...
  const maxQuantity = Math.min(userMaxSlots, offer.quantity_available); // Use user's max slots
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {offer.images && offer.images.length > 0 && (
          <HeaderImage
            imageUrl={resolveOfferImageUrl(offer.images[0], offer.category, { width: 800, quality: 85 })}
            title={offer.title}
        ...
        <div className="space-y-4 bg-white rounded-b-2xl">
          ...
          <UnifiedPriceCard
            ...
            onUnlockClick={() => setShowSlotUnlockModal(true)}
      ...
      {user && (
        <SlotUnlockModal
          open={showSlotUnlockModal}
          onOpenChange={setShowSlotUnlockModal}
          userId={user.id}
          currentMax={userMaxSlots}
          currentBalance={pointsBalance}
          onSuccess={handleSlotUnlockSuccess}
        />
      )}
    </Dialog>
  );
