...
export default function PartnerOffersModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] p-0 gap-0 overflow-hidden rounded-3xl bg-[#1a1a1a] border border-gray-800">
        {/* Modern Partner Header with teal/mint gradient */}
        <DialogHeader className="px-5 pt-5 pb-5 bg-gradient-to-br from-teal-500 to-mint-500 text-white space-y-0 relative">
          {/* Close Button - Top Right */}
          <button
            onClick={() => onOpenChange(false)}
            className="absolute top-4 right-4 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full bg-white/20 hover:bg-white/30 transition-colors z-10"
            aria-label="Close"
          >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {/* Rating Badge */}
            <div className="inline-flex items-center gap-1.5 bg-white/20 backdrop-blur-sm rounded-lg px-3 py-1.5">
        ...
        <div className="overflow-y-auto px-5 py-5 bg-[#1a1a1a]" style={{ maxHeight: 'calc(90vh - 180px)' }}>
          <div className="grid grid-cols-2 gap-4">
            {offers.map((offer) => {
              const imageUrl = offer.images && offer.images.length > 0
                ? resolveOfferImageUrl(offer.images[0], offer.category, { width: 400, quality: 80 })
                : '/placeholder.png';
              ...
              return (
                <div
                  ...
                  className="cursor-pointer group bg-[#2a2a2a] rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-gray-800"
                >
                  {/* Food Image Card */}
                  ...
                  <div className="p-3">
                    {/* Food Title */}
                    <h3 className="text-sm font-bold text-white mb-2 line-clamp-2 leading-tight min-h-[36px]">
                      {offer.title}
                    </h3>
                    ...
                    <Badge 
                      ...
                      className={`text-xs font-semibold ${lowStock ? 'bg-red-500/20 text-red-400 border-red-500/50' : 'bg-gray-700 text-gray-300 border-gray-600'}`}
                    >
