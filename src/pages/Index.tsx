...
export default function Index() {
  ...
  const checkUser = async () => {
    ...
    // Move helper and handlers before usage

    function getFilteredAndSortedOffers(): Offer[] {
      let filtered = [...offers];
    ...
    function getDistanceToOffer(offer: Offer) {
    ...
    return (
      <>
        ...
        <CategoryTabs selectedCategory={selectedCategory} onCategorySelect={setSelectedCategory} />
        <main className="container mx-auto px-2 md:px-4 pt-2 pb-8">
          ...
          <div className="w-full flex justify-center items-center py-3">
            <input
              ...
              className="w-full max-w-md px-4 py-2 rounded-full border border-gray-300 shadow focus:outline-none focus:ring-2 focus:ring-[#00C896]"
            />
          </div>
          {/* Map Section always visible */}
          <MapSection
            offers={getFilteredAndSortedOffers()}
            onOfferClick={handleOfferClick}
            selectedCategory={selectedCategory}
            onCategorySelect={setSelectedCategory}
            onLocationChange={setUserLocation}
          />
          {/* Point-based item list below map, styled as cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-8">
            {getFilteredAndSortedOffers().map(offer => (
              <div key={offer.id} className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col">
                <img src={offer.images && offer.images.length > 0 ? resolveOfferImageUrl(offer.images[0], offer.category) : '/placeholder.png'} alt={offer.title} className="w-full h-40 object-cover" />
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-400 mb-2">
                      <span><MapPin className="inline w-4 h-4 mr-1" />{getDistanceToOffer(offer) ? `${getDistanceToOffer(offer)}km` : ''}</span>
                      <span>{offer.pickup_start && offer.pickup_end ? `${new Date(offer.pickup_start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - ${new Date(offer.pickup_end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ''}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-2">
                    <span className="font-bold text-[#00C896] text-xl">{offer.smart_price} pts</span>
                    <Button size="sm" className="ml-2" onClick={() => handleOfferClick(offer)}>View</Button>
                  </div>
                </div>
              </div>
            ))}
          </div>
          {/* ...existing code for modals, alerts, etc... */}
        </main>
        <AuthDialog open={showAuthDialog} defaultTab={defaultAuthTab} onOpenChange={setShowAuthDialog} />
        <ReservationModal open={showReservationModal} offer={selectedOffer} user={user} onOpenChange={setShowReservationModal} />
        <FilterDrawer
