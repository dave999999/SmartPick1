...
export default function MyPicks() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#0a0a0a] to-[#1a1a1a] flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 md:h-32 md:w-32 border-b-2 border-[#00cc66] mx-auto"></div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a0a] to-[#1a1a1a] safe-area-top">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#2a2a2a] to-[#1a1a1a] backdrop-blur-md border-b border-white/10 shadow-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-3 md:py-4">
            <div className="flex items-center gap-2 md:gap-4">
              <Button
                variant="ghost"
                className="h-9 md:h-11 text-gray-300 hover:text-white hover:bg-white/10"
                onClick={() => navigate('/')}
              >
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-white">{t('mypicks.title')}</h1>
                <p className="text-xs md:text-sm text-gray-600 hidden md:block">{t('mypicks.subtitle')}</p>
              </div>
            </div>
              ...
              <>
                {!hasPermission ? (
                  <Button
                    variant="outline"
                    size="default"
                    className="h-9 md:h-11 gap-2 border-teal-300 text-teal-700 hover:text-teal-900 hover:bg-teal-50 hidden sm:flex"
                  ...
                  <div className="hidden sm:flex items-center gap-2 text-sm text-green-700 bg-green-50 px-3 md:px-4 py-2 rounded-lg border border-green-300">
                    <Bell className="h-4 w-4" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-8">
        <Tabs defaultValue="active" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 bg-[#2a2a2a] border border-white/10">
            <TabsTrigger value="active" className="data-[state=active]:bg-[#00cc66] data-[state=active]:text-black text-gray-400">
              {t('mypicks.tab.active')} ({activeReservations.length})
            </TabsTrigger>
            <TabsTrigger value="history" className="data-[state=active]:bg-[#00cc66] data-[state=active]:text-black text-gray-400">
          ...
          <TabsContent value="active" className="space-y-4">
            {activeReservations.length === 0 ? (
              <Card className="bg-gradient-to-br from-[#2a2a2a] to-[#1a1a1a] border border-white/5">
                <CardContent className="flex flex-col items-center justify-center py-16 px-6">
                  <div className="relative mb-6">
                    <div className="absolute inset-0 bg-[#00cc66] rounded-full blur-2xl opacity-20 animate-pulse"></div>
                    <div className="relative bg-gradient-to-br from-[#3a3a3a] to-[#2a2a2a] rounded-full p-6 shadow-lg">
                      <Clock className="h-16 w-16 text-[#00cc66]" />
                    </div>
                  </div>
                  ...
                  <Button 
                    onClick={() => navigate('/')} 
                    className="bg-[#00cc66] hover:bg-[#00ff7f] text-black font-bold px-8 py-6 text-lg rounded-xl shadow-lg hover:shadow-xl transition-all"
                  >
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {activeReservations.map((reservation) => (
                  <Card 
                    key={reservation.id} 
                    className="relative cursor-pointer transition-all hover:shadow-2xl hover:scale-105 active:scale-[0.99] bg-gradient-to-br from-[#2a2a2a] to-[#1a1a1a] border border-white/5 shadow-xl"
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          {reservation.offer?.images?.[0] ? (
                            <img
                              src={resolveOfferImageUrl(reservation.offer.images[0], reservation.offer.category, { width: 100, quality: 80 }) || '/images/Map.jpg'}
                          ...
                          <div>
                            ...
                            <Popover>
                              <PopoverTrigger asChild>
                                <Button variant="link" className="p-0 h-auto text-sm text-[#00cc66] hover:text-[#00ff7f] font-medium">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 w-full">
                          <Button
                            ...
                            className="h-11 bg-[#00cc66] hover:bg-[#00ff7f] text-black font-semibold w-full"
                          >
                          ...
                          {reservation.status === 'ACTIVE' && (
                            <>
                              <Button
                                ...
                                className="h-11 w-full border-white/20 text-white hover:bg-white/10"
                              >
          ...
          <TabsContent value="history" className="space-y-4">
            {historyReservations.length === 0 ? (
              <Card className="bg-gradient-to-br from-[#2a2a2a] to-[#1a1a1a] border border-white/5">
                <CardContent className="flex flex-col items-center justify-center py-16 px-6">
                  <div className="relative mb-6">
                    <div className="absolute inset-0 bg-[#00cc66] rounded-full blur-2xl opacity-20"></div>
                    <div className="relative bg-gradient-to-br from-[#3a3a3a] to-[#2a2a2a] rounded-full p-6 shadow-lg">
                  ...
                  >
                    <CardHeader className="pb-3">
                      <Button
                        variant="ghost"
                        size="icon"
                        className="absolute right-2 top-2 hover:bg-red-500/20 text-red-400 hover:text-red-300"
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          {reservation.offer?.images?.[0] ? (
                            <img
                              src={resolveOfferImageUrl(reservation.offer.images[0]) || '/images/Map.jpg'}
                              alt={reservation.offer.title}
                              className="w-12 h-12 rounded-lg object-cover"
                              onError={(e) => { (e.currentTarget as HTMLImageElement).src = '/images/Map.jpg'; }}
                            />
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <span className="text-gray-400">{t('mypicks.quantity')}</span>
                          <p className="font-medium text-white">{reservation.quantity}</p>
                        </div>
                        <div>
                          <span className="text-gray-400">{t('mypicks.total')}</span>
                          <p className="font-bold text-[#00cc66]">{formatPrice(reservation.total_price || 0)}</p>
                        </div>
                      </div>
                      
                      {reservation.picked_up_at && (
                        <div className="text-sm">
                          <span className="text-gray-400">{t('label.pickedUpAt')}:</span>
                          <p className="font-medium text-white">{formatDateTime(reservation.picked_up_at)}</p>
                        </div>
                      )}
                        ...
                        {reservation.status === 'PICKED_UP' && (
                          <>
                            <Button
                              ...
                              className="h-11 flex-1 sm:flex-none border-white/20 text-white hover:bg-white/10"
                            >
        ...
        {user && (
          <Card className="mt-8 bg-gradient-to-br from-[#2a2a2a] to-[#1a1a1a] border border-white/5">
            <CardHeader>
